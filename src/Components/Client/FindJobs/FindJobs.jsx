...
import { getFullJobs, saveJob } from "../../../Services/post"
...
const FindJobs = () => {
    ...
    const manageJobSave = async (job_id, user_id) => {
        const response = await saveJob(job_id, user_id)
        if(response){
            if(!myData.saved_posts.includes(job_id)){
                setMyData({...myData, saved_posts: [...myData.saved_posts, job_id]})
            }else{
                myData.saved_posts.splice(myData.saved_posts.indexOf(job_id),1)
        ...
        <>
            ...
            <div className="px-2 md:px-10 mt-20 grid gap-3 grid-cols-12">
                ...
                <div className="col-span-12 md:col-span-8 lg:col-span-9 font-primary rounded-xl relative">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="md:hidden group pr-3 shadow shadow-gray-500 p-3 mr-2 rounded-xl mb-2 flex justify-center items-center cursor-pointer">
                            ...
                            <section className="absolute bg-white top-16 left-[-30rem] duration-500 transition-all group-hover:left-0 w-[250px]">
                                <Filter visible={"md:hidden absolute z-10 top-[-1rem] bg-white"} actions={{filters, setFilters}} query={query}/>
                            </section>
                        </div>
                    ...
                    {
                        isLoading ? 
                            <div className=" absolute top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%]"><Loading/></div> 
                        : jobs.length==0 ? 
                                ...
                                return(
                                    <div key={items._id} className="w-full shadow shadow-gray-500 mb-2 rounded-xl p-2">
                                        <div className="flex justify-between items-baseline">
                                            ...
                                            <div onClick={async ()=>await manageJobSave(items._id, id)} className="w-[30px] h-[30px] flex-shrink-0 flex justify-center items-center rounded-full bg-gray-800 bg-opacity-70 cursor-pointer">
                                                <i className={`${myData?.saved_posts?.includes(items._id) ? `fa` : `far`} fa-bookmark text-white`}></i>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex gap-1 flex-wrap items-center mt-3">
                                            Tags: {
                                                items.tags.map(item => {
                                                    return(
                                                        <div key={item} className="bg-red-700 cursor-pointer bg-opacity-60 text-white rounded-xl p-0.5 px-2">{item}</div>
                                                    )
                                                })
                                            }
                                        </div>
